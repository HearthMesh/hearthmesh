...
func main() {
	if err := run(os.Args[1:]); err != nil { fmt.Fprintln(os.Stderr, "error:", err); os.Exit(1) }
}

func run(args []string) error {
	if len(args) == 0 { return errors.New("usage: hearthmesh <identity|pack|verify|publish|serve> [flags]") }
	switch args[0] {
	case "identity":
	...
	case "pack":
		...
		if err := fs.Parse(args[1:]); err != nil { return err }
		if *src == "" || *out == "" { return errors.New("-src and -out are required") }
		key, err := hearthmesh.LoadIdentity(*keyPath)
		if err != nil { return err }
	...
	case "verify":
		...
		if err := fs.Parse(args[1:]); err != nil { return err }
		if *file == "" { return errors.New("-pack is required") }
	...
	case "publish":
		...
		if res.StatusCode != http.StatusCreated { body, _ := io.ReadAll(io.LimitReader(res.Body, 4096)); return fmt.Errorf("node returned %d: %s", res.StatusCode, body) }
	...
	case "serve":
		...
		if err := fs.Parse(args[1:]); err != nil { return err }
		if *interval < time.Second { return errors.New("-sync must be at least 1s") }
		if err := os.MkdirAll(filepath.Clean(*data), 0700); err != nil { return err }
		...
		go func() { <-ctx.Done(); shutdown, cancel := context.WithTimeout(context.Background(), 5*time.Second); defer cancel(); _ = srv.Shutdown(shutdown) }()
		fmt.Printf("experimental node %s listening on %s\n", n.ID, *listen)
		if err := srv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) { return err }
	default:
